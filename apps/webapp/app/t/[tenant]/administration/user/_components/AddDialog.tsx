...
export function AddDialog({ open, onClose, onSave }: AddDialogProps) {
  ...
  useEffect(() => {
    if (!open) form.reset({ items: [{ role: "", email: "", phone: "", password: "", status: false }] });
  }, [open, form]);
  ...
  return (
    <AppDialog open={open} onClose={onClose} title="Add New User" maxWidth="md:max-w-1xl">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSave)} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-6 border-t">
            <Button type="button" variant="outline" onClick={onClose} className="text-blue-600 border-blue-500">
              Cancel
            </Button>
            <Button type="submit" disabled={submitting} className="bg-green-500 hover:bg-green-600 text-white">
