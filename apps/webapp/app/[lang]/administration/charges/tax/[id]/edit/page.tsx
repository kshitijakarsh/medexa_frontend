...
import { getTaxById, updateTax } from "../../../_components/api";
...
function EditTaxPageContent() {
  ...
  useEffect(() => {
    if (!id) {
      setError("Invalid tax category ID");
      setLoading(false);
      return;
    }
    ...
    const loadTax = async () => {
      try {
        ...
        if (!tax) {
          setError("Tax category not found");
          setLoading(false);
          return;
        }

        // Pre-populate form with existing tax category data
        form.reset({
          name: tax.taxName || "",
          percentage: tax.percentage || 0,
          active: tax.status === "Active",
        });
        setLoading(false);
      } catch (err) {
        console.error("Error loading tax category:", err);
        setError("Failed to load tax category data");
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (vals: any) => {
    ...
    try {
      const payload = {
        taxName: vals.name,
        percentage: Number(vals.percentage) || 0,
      ...
      setError(err.message || "Failed to update tax category");
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <main className="min-h-screen w-full bg-gradient-to-br from-[#ECF3FF] to-[#D9FFFF]">
        <Header />
        <div className="p-5">
          <PageHeader title="Edit Tax Category" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField name="name" control={form.control} render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="Enter Tax Name" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
