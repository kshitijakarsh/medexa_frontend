...
class RegulatoryApiClient {
  ...
  private getMultipartRequestConfig(): AxiosRequestConfig {
    return {
      headers: {
        // Don't set Content-Type for multipart/form-data - axios will set it automatically
        // with the correct boundary parameter when FormData is used
        Accept: "application/json",
        Authorization: `Bearer ${this.authToken}`,
      },
    }
  ...
  async uploadDocument(file: File): Promise<string> {
    try {
      ...
      // Try to upload file - this endpoint might not exist yet
      // If it doesn't exist, we'll use the file name as a placeholder
      try {
        const response = await axios.post<{ file_url: string }>(
          `${this.baseUrl}/api/v1/upload/document`,
          formData,
          this.getMultipartRequestConfig()
        )
      ...
      if (axios.isAxiosError(error)) {
        ...
        // If upload fails, return file name as fallback
        return file.name
      }
      // If it's not an axios error, return file name as fallback
      return file.name
    }
  }
