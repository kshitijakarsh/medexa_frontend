...
type ModuleStepFormProps = {
  dict: Dictionary
}

export function ModuleStepForm({ dict }: ModuleStepFormProps) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const params = useParams<{ lang: string }>()
  const queryClient = useQueryClient()
  ...
  useEffect(() => {
    if (tenantData?.tenant_modules) {
      const moduleIds = tenantData.tenant_modules.map((m) => String(m.id))
      if (
  ...
  useEffect(() => {
    ...
    if (current.length === 0 && moduleState.selectedIds.length > 0) {
      form.setValue("modules", moduleState.selectedIds, {
        shouldDirty: false,
        shouldValidate: false,
      })
    }
  }, [moduleState.selectedIds])
  ...
  const mutation = useMutation({
    mutationFn: async (ids: string[]) => {
      const client = createModulesApiClient({ authToken })
      return client.updateModules(hospitalId, {
        moduleIds: ids.map(Number),
      })
    },
    onSuccess: () => {
      saveModules()
      const params = new URLSearchParams({ hospitalId })
      if (isEditMode) params.set("type", "edit")
      router.push(`${onboardingBase}/payment?${params.toString()}`)
    },
  })
  ...
  return (
    <Form {...form}>
      <form className="space-y-4" noValidate>
        ...
        <div className="bg-white/80 rounded-lg p-4 md:p-6 flex flex-col md:flex-row items-center relative">
          {mutation.isError && (
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full mb-2 text-sm text-red-600 bg-red-50 px-4 py-2 rounded-md border">
              {mutation.error instanceof Error
                ? mutation.error.message
                : dict.common.error}
            </div>
          ...
          >
            ...
            <div className="flex gap-3 items-center">
              ...
              <Button
                type="button"
                onClick={() => mutation.mutate(selectedModules)}
                disabled={mutation.isPending}
