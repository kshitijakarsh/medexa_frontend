...
export default function OnboardWizard({ dict }: { dict: Dictionary }) {
  const [currentStep, setCurrentStep] = useState(0)
  const [tenantId, setTenantId] = useState<string | null>(null)
  const [loading, setLoading] = useState(false)
  const [serverError, setServerError] = useState<string | null>(null)
  ...
  const handlePrimary = async () => {
    ...
    if (!(await form.trigger())) {
      setServerError(dict.common.loading)
      return
    }
    ...
    try {
      if (currentStep === 0) {
        const res = await tenantApi.createTenant(step1Form.getValues())
        setTenantId(String(res.data.data.id))
        setCurrentStep(1)
      } else if (currentStep === 1) {
        await modulesApi.updateModules(tenantId!, {
          moduleIds: step2Form.getValues().modules.map(Number),
        })
        setCurrentStep(2)
      } else if (currentStep === 2) {
        await paymentApi.createPaymentConfig(
          tenantId!,
          step3Form.getValues()
        )
        setCurrentStep(3)
      } else if (currentStep === 3) {
        await licenseApi.createLicense(tenantId!, step4Form.getValues())
        setCurrentStep(4)
      } else {
        await regulatoryApi.createDocument(
          tenantId!,
          step5Form.getValues()
        )
        window.location.href = "/hospitals"
  ...
  return (
    <main className="min-h-svh w-full">
      ...
      {currentStep === 1 && (
        <Form {...step2Form}>
          <ModuleAssignmentSection form={step2Form} dict={dict} />
        </Form>
      )}

      {currentStep === 2 && (
        <Form {...step3Form}>
          <PaymentDetailsSection form={step3Form} dict={dict} />
        </Form>
      )}
