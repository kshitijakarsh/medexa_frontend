...
export function ModuleAssignmentSection({
  ...
  const t = dict.pages.onboarding.modulesAssignment
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (!newOpen && isSelectingRef.current) return
    setOpen(newOpen)
    if (!newOpen) isSelectingRef.current = false
  }
              ...
              <div className="flex flex-wrap gap-2">
                {selectedModules.map((id: string) => {
                  ...
                  return (
                    ...
                    >
                      {mod?.name_en ||
                        t.fallbackModule.replace("{{id}}", id)}
